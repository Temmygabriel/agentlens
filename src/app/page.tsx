...
type Stats={total:number;live:number;offline:number;slow:number;capabilityTypes:number;protocolTypes:number;domainTypes:number;withCapabilities:number};
const PAGE_SIZE=24;
const badge=(s:string|null)=>s==="LIVE"?{c:"live",t:"LIVE"}:s==="DEAD"?{c:"off",t:"OFFLINE"}:s==="TIMEOUT"?{c:"slow",t:"SLOW"}:{c:"unv",t:"UNVERIFIED"};
...
export default function Home(){
 ...
 useEffect(()=>{fetch("/api/stats").then(r=>r.json()).then(d=>{if(d&&typeof d.total==="number")setStats(d)}).catch(()=>{})},[]);
 ...
 return <div className="shell">
  <nav className="nav"><div className="container" style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><Link href="/" className="brand"><span className="brand-mark">A</span>AgentLens</Link><div style={{display:"flex",alignItems:"center",gap:22}}><Link href="/" className="nav-link nav-link-active">Explore</Link><Link href="/discover" className="nav-link">Discover</Link><Link href="/diversity" className="nav-link">Diversity</Link><span className="nav-note">BNB Smart Chain · ERC-8004</span></div></div></nav>
  <main className="container">
   <section className="hero"><div className="eyebrow"><span className="eyebrow-dot"/>Verified agent discovery</div><h1>Find an agent you can actually use.</h1><p>Discover AI agents on BNB Smart Chain, compare what they do, and inspect evidence before you put them to work.</p><div className="search-wrap"><span className="search-icon">⌕</span><input className="search" value={q} onChange={e=>setQ(e.target.value)} placeholder="Search by agent name, capability, or description…"/></div>{stats?<div className="proofbar"><span className="proof-live"><span className="scan-dot"/><span><strong>{stats.live}</strong> of {stats.total} agents reachable right now</span></span><span className="proof-sep"/><span className="proof-mini"><i className="d d-off"/><strong>{stats.offline??0}</strong> offline</span><span className="proof-mini"><i className="d d-unv"/><strong>{Math.max(stats.total-stats.live-(stats.slow??0)-(stats.offline??0),0)}</strong> unverified</span><span className="proof-sep"/><span className="proof-mini"><strong>{stats.capabilityTypes}</strong> skill types</span><span className="proof-mini"><strong>{stats.protocolTypes}</strong> protocols</span></div>:<div className="stats"><div className="stat"><strong>{loading?"—":total}</strong> agents found</div><div className="stat"><strong>BNB</strong> Smart Chain</div><div className="stat"><strong>ERC-8004</strong> identity</div></div>}</section>
