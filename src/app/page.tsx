...
type Stats={total:number;live:number;capabilityTypes:number;protocolTypes:number;domainTypes:number;withCapabilities:number};
const PAGE_SIZE=24;
...
export default function Home(){
 ...
 return <div className="shell">
  ...
  <main className="container">
   <section className="hero"><div className="eyebrow"><span className="eyebrow-dot"/>Verified agent discovery</div><h1>Find an agent you can actually use.</h1><p>Discover AI agents on BNB Smart Chain, compare what they do, and inspect evidence before you put them to work.</p><div className="search-wrap"><span className="search-icon">⌕</span><input className="search" value={q} onChange={e=>setQ(e.target.value)} placeholder="Search by agent name, capability, or description…"/></div><div className="stats">{stats?<><div className="stat"><strong>{stats.total}</strong> agents indexed</div><div className="stat stat-live"><strong>{stats.live}</strong> live right now</div><div className="stat"><strong>{stats.capabilityTypes}</strong> skill types</div><div className="stat"><strong>{stats.protocolTypes}</strong> protocols</div></>:<><div className="stat"><strong>{loading?"—":total}</strong> agents found</div><div className="stat"><strong>BNB</strong> Smart Chain</div><div className="stat"><strong>ERC-8004</strong> identity</div></>}</div></section>
