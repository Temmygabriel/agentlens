...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase
      ...
      // Pool size the ranking/filtering step draws from before pagination
      // is applied client-side via offset/limit below. 300 comfortably
      // covers the full current agent population; raise if it grows.
      .limit(300);
