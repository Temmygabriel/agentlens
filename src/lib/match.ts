...
export type MatchInput = { name?: string|null; description?: string|null; metadata?: unknown; health_status?: string|null };
...
export function rankAgents(query:string,agents:MatchInput[]){const q=norm(query),ts=terms(query);return agents.map(agent=>{const name=norm(agent.name??""),desc=norm(agent.description??""),meta=norm(flat(agent.metadata).join(" ")),text=`${name} ${desc} ${meta}`;let score=0;const reasons:string[]=[];if(q&&name===q){score+=60;reasons.push("Exact name match")}else if(q&&name.includes(q)){score+=35;reasons.push("Name matches your request")}for(const t of ts)if(text.includes(t)){score+=name.includes(t)?14:meta.includes(t)?10:7;if(reasons.length<3)reasons.push(`Matches “${t}”`)}score+=Math.min(CAPABILITIES.filter(c=>c.terms.some(t=>text.includes(t))).length*2,10);if(agent.health_status==="LIVE"){score+=8;reasons.push("Currently reachable")}else if(agent.health_status==="TIMEOUT")score+=2;return{agent,matchScore:Math.min(100,Math.round(score)),matchReasons:[...new Set(reasons)].slice(0,4)}}).sort((a,b)=>b.matchScore-a.matchScore)}
